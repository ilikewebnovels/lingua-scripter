...
export const useCharactersDB = () => {
  ...
  useEffect(() => {
    const fetchCharacterDBs = async () => {
      try {
        const response = await authFetch(`${API_URL}/characters/all`);
  ...
  const saveCharacterDBToServer = useCallback(async (projectId: string, characterData: Character[]) => {
    try {
      const response = await authFetch(`${API_URL}/characters/${projectId}`, {
        method: 'PUT',
  ...
  const deleteCharacterDB = useCallback(async (projectId: string) => {
    ...
    try {
      const response = await authFetch(`${API_URL}/characters/${projectId}`, { method: 'DELETE' });
