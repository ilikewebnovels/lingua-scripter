...
export const useGlossary = () => {
  ...
  useEffect(() => {
    const fetchGlossaries = async () => {
      try {
        const response = await authFetch(`${API_URL}/glossaries/all`);
  ...
  const saveGlossaryToServer = useCallback(async (projectId: string, glossaryData: GlossaryEntry[]) => {
    try {
      const response = await authFetch(`${API_URL}/glossaries/${projectId}`, {
        method: 'PUT',
  ...
  const deleteGlossary = useCallback(async (projectId: string) => {
    ...
    try {
      const response = await authFetch(`${API_URL}/glossaries/${projectId}`, { method: 'DELETE' });
