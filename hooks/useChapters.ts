...
export const useChapters = () => {
  ...
  const fetchChaptersByProject = useCallback(async (projectId: string | null) => {
    ...
    try {
      const response = await authFetch(`${API_URL}/chapters/project/${projectId}`);
  ...
  const fetchAllChapters = useCallback(async () => {
    try {
      const response = await authFetch(`${API_URL}/chapters`);
      if (!response.ok) throw new Error('Failed to fetch chapters');
  ...
  const updateChapter = useCallback(async (id: string, updates: Partial<Omit<Chapter, 'id' | 'projectId' | 'createdAt'>>) => {
    try {
      const response = await authFetch(`${API_URL}/chapters/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteChapter = useCallback(async (id: string) => {
    try {
      const response = await authFetch(`${API_URL}/chapters/${id}`, { method: 'DELETE' });
  ...
  const deleteChaptersByProjectId = useCallback(async (projectId: string) => {
    try {
      const response = await authFetch(`${API_URL}/chapters/by-project/${projectId}`, { method: 'DELETE' });
  ...
  const addChaptersBatch = useCallback(async (
    ...
    try {
      ...
      for (let i = 0; i < chaptersToSend.length; i += CHUNK_SIZE) {
        ...
        const response = await authFetch(`${API_URL}/chapters/batch`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
