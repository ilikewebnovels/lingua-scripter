...
import { translateText, translateTextStream, testConnection, findOriginalPhrases, analyzeForCharacters, translateBatchStream } from './services/geminiService';
...
const PasswordChangeModal = React.lazy(() => import('./components/PasswordChangeModal'));
...
function App() {
  ...
  const [autoStartBatch, setAutoStartBatch] = useState(false);
  ...
  const [quickBatchNotification, setQuickBatchNotification] = useState<string | null>(null);
  ...
  // Auto-dismiss quick batch notification
  useEffect(() => {
    if (quickBatchNotification) {
      const timer = setTimeout(() => setQuickBatchNotification(null), 4000);
      return () => clearTimeout(timer);
    }
  }, [quickBatchNotification]);
  ...
  // Quick batch translate handler for reading mode - translates directly without opening modal
  const handleQuickBatchTranslate = useCallback(async () => {
    // Count untranslated chapters
    const untranslatedChapters = projectChapters.filter(ch => !ch.translatedText?.trim());
    const batchCount = Math.min(settings.batchSize, untranslatedChapters.length);

    if (batchCount === 0) {
      setQuickBatchNotification('All chapters are already translated!');
      return;
    }

    if (connectionStatus !== 'connected') {
      setQuickBatchNotification('Please connect to API first');
      return;
    }

    if (isBatchTranslating) {
      setQuickBatchNotification('Batch translation already in progress');
      return;
    }

    // Get chapters to translate
    const chaptersToTranslate = untranslatedChapters.slice(0, settings.batchSize).map(ch => ({
      id: ch.id,
      title: ch.title,
    ...
    // Show notification
    setQuickBatchNotification(`Translating ${chaptersToTranslate.length} chapter${chaptersToTranslate.length > 1 ? 's' : ''}...`);

    // Start batch translation (parent state)
    const chapterIds = chaptersToTranslate.map(ch => ch.id);
    handleBatchTranslationStart(chapterIds);
    ...
    try {
      ...
      for await (const progress of stream) {
        if (progress.type === 'chapter_complete' && progress.chapterId && progress.fullText) {
          // Save completed chapter
          await updateChapter(progress.chapterId, {
            translatedText: progress.fullText
          });
          // If this is the currently viewed chapter, update the local state directly
          if (progress.chapterId === activeChapterId) {
          ...
          // Update notification
          setQuickBatchNotification(`Translated ${translatedCount}/${chaptersToTranslate.length} chapters`);
        } else if (progress.type === 'chunk' && progress.chapterIndex && progress.text) {
          ...
          setQuickBatchNotification('Translation error: ' + (progress.error || 'Unknown error'));
          break;
        }
      }
      ...
      if (translatedCount > 0) {
        setQuickBatchNotification(`✓ Translated ${translatedCount} chapter${translatedCount > 1 ? 's' : ''}`);
      }
    } catch (err) {
      setQuickBatchNotification('Translation failed: ' + (err instanceof Error ? err.message : 'Unknown error'));
    }

    handleBatchTranslationComplete();
  }, [projectChapters, settings, connectionStatus, isBatchTranslating, activeGlossary, activeCharacterDB, updateChapter, handleBatchTranslationStart, handleBatchTranslationProgress, handleBatchTranslationComplete]);
  ...
  const handleSelectProject = (id: string | null) => {
    ...
    // Auto-load last chapter if available
    if (id) {
      const project = projects.find(p => p.id === id);
      if (project?.lastChapterId) {
        setActiveChapterId(project.lastChapterId);
      } else {
        setActiveChapterId(null);
      }
    } else {
  ...
  return (
    <div className="h-screen text-[var(--text-primary)] flex font-sans bg-[var(--bg-primary)] overflow-hidden">
      ...
      {activeProjectId && isSidebarOpen && (
        <div
          className="mobile-overlay md:hidden active"
        ...
        <header className="flex items-center justify-between gap-4 p-4 border-b border-[var(--border-primary)] bg-[var(--bg-secondary)]/80 backdrop-blur-sm">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <span className="hidden sm:block"><LogoIcon /></span>
              <div>
                <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-[var(--text-primary)] flex items-center gap-1 sm:gap-2 flex-wrap">
                  <span className="whitespace-nowrap">Lingua Scripter</span>
                  <span className="hidden sm:inline text-xs font-medium bg-[var(--bg-tertiary)] text-[var(--text-secondary)] px-2 py-0.5 rounded-full">by Subscribe</span>
                  <a
                    href="https://ko-fi.com/subscribe123"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hidden sm:flex text-xs font-medium bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-[var(--text-on-accent)] px-2 py-0.5 rounded-full items-center gap-1 transition-colors"
                ...
                <p className="hidden sm:block text-xs text-[var(--text-secondary)]">AI-Powered Novel Translation</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2 md:gap-4">
            <ConnectionStatus status={connectionStatus} error={connectionError} />
            {/* Batch Translate Button - visible when project is active */}
            {activeProjectId && (
              <button
                onClick={() => setIsBatchTranslateOpen(true)}
                className="text-xs sm:text-sm bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-[var(--text-on-accent)] font-semibold px-2 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap"
            ...
            <button
              ...
              className="text-xs sm:text-sm bg-[var(--bg-tertiary)] hover:brightness-125 font-semibold px-2 sm:px-3 py-1.5 rounded-md transition-all disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
            ...
            <button onClick={() => setIsPasswordChangeOpen(true)} className="p-2 rounded-md text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)] transition-colors" aria-label="Change password" title="Change Password">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
              </svg>
            </button>
            <button onClick={logout} className="p-2 rounded-md text-[var(--text-secondary)] hover:bg-red-500/20 hover:text-red-400 transition-colors" aria-label="Logout" title="Logout">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <>
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1 sm:gap-2 min-w-[44px] sm:min-w-[120px] md:min-w-[160px] font-semibold px-2 sm:px-4 py-2 rounded-md transition-colors disabled:cursor-not-allowed
                                ${saveStatus === 'saved'
                      ...
                      : 'bg-[var(--accent-primary)] hover:bg-[var(--accent-secondary)] text-[var(--text-on-accent)] disabled:bg-gray-600'
                            ...
                            `}
                >
                  {saveStatus === 'idle' && <><SaveIcon /> <span className="hidden sm:inline">Save</span><span className="hidden md:inline"> Chapter</span></>}
      ...
      <Suspense fallback={<ModalLoader />}>
        {isPasswordChangeOpen && (
          <PasswordChangeModal isOpen={isPasswordChangeOpen} onClose={() => setIsPasswordChangeOpen(false)} />
        )}
      </Suspense>
      ...
      <Suspense fallback={<ModalLoader />}>
        {isBatchTranslateOpen && activeProjectId && (
          <BatchTranslateModal
            ...
            // Auto-start props for reading mode quick batch
            autoStartNextBatch={autoStartBatch}
            onAutoStartConsumed={() => setAutoStartBatch(false)}
          />
        )}
      </Suspense>

      {
        isTranslatedFullscreen && (
          <div className="fixed inset-0 z-50 bg-[var(--bg-primary)] flex flex-col p-4 md:p-8 animate-fade-in">
            ...
            {/* Quick batch notification toast */}
            {quickBatchNotification && (
              <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-[var(--accent-primary)] text-[var(--text-on-accent)] px-4 py-2 rounded-lg shadow-lg animate-fade-in text-sm font-medium">
                {quickBatchNotification}
              </div>
            )}
