...
interface BatchTranslateModalProps {
    ...
    isConnected: boolean;
    // Auto-start next batch when modal opens (for reading mode quick batch)
    autoStartNextBatch?: boolean;
...
const BatchTranslateModal: React.FC<BatchTranslateModalProps> = ({
    ...
    // Auto-start translation when modal opens with autoStartNextBatch
    useEffect(() => {
        if (isOpen && autoStartNextBatch && isConnected && !isBatchTranslating && !hasAutoStarted.current) {
            hasAutoStarted.current = true;
            // Use setTimeout to allow modal to render first
            const timer = setTimeout(() => {
                // Trigger the next batch translation
                const nextBatch = sortedChapters.filter(ch => !ch.translatedText?.trim()).slice(0, settings.batchSize);
                if (nextBatch.length > 0) {
                    // Call onAutoStartConsumed to reset the flag in parent
                    onAutoStartConsumed?.();
                    // Simulate clicking the "Translate Next Batch" button by triggering the same logic
                    document.querySelector<HTMLButtonElement>('[data-auto-batch-trigger]')?.click();
                }
            }, 100);
    ...
    }, [isOpen, autoStartNextBatch, isConnected, isBatchTranslating, sortedChapters, settings.batchSize, onAutoStartConsumed]);
