      ...
      <div className={`border-b border-[var(--border-primary)] bg-[var(--bg-secondary)] transition-all duration-300 ${isHeaderCollapsed ? 'py-3 px-6' : 'p-6 pb-4'}`}>
        ...
        {!isHeaderCollapsed && (
          <>
            ...
            {/* Quick Actions */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-4 ml-0 sm:ml-10">
              <button
          ...
          >
            {filteredProjects.map((project, index) => (
              editingProjectId === project.id ? (
                ...
                <div
                  key={project.id}
                  className={`w-full group flex items-center gap-4 sm:gap-6 p-4 bg-[var(--bg-secondary)] border rounded-xl hover:border-[var(--accent-primary)] hover:shadow-lg hover:shadow-[var(--accent-primary)]/10 transition-all duration-300 overflow-hidden ${index === selectedIndex
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-4 sm:gap-6 flex-1 min-w-0 text-left overflow-hidden"
                  >
                    <div className="flex-shrink-0 w-20 h-20 sm:w-32 sm:h-32 rounded-lg bg-[var(--bg-tertiary)] overflow-hidden flex items-center justify-center border border-[var(--border-primary)] group-hover:border-[var(--accent-primary)] transition-colors">
                      {project.profilePic ? (
                        <img
                    ...
                    <div className="w-0 flex-grow">
                      <h3 className="text-lg sm:text-xl font-bold text-[var(--text-primary)] group-hover:text-[var(--accent-primary)] transition-colors truncate">
                        {project.name}
                      </h3>
                      ...
                      {project.lastChapterTitle && (
                        <p className="text-xs text-[var(--accent-primary)] mt-1 truncate">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
              {filteredProjects.map((project) => (
                editingProjectId === project.id ? (
                  ...
                  <div
                    key={project.id}
                    className="group flex flex-col items-center gap-3 p-4 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-xl hover:border-[var(--accent-primary)] hover:shadow-lg hover:shadow-[var(--accent-primary)]/10 transition-all duration-300 cursor-pointer relative overflow-hidden"
                    onClick={() => onSelectProject(project.id)}
                  >
                    ...
                    <div className="text-center w-full overflow-hidden">
                      <h3 className="font-semibold text-sm text-[var(--text-primary)] truncate break-words">{project.name}</h3>
